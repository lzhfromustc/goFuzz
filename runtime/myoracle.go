...
type ChanInfo struct {
	...
	mapRefGoroutine map[*GoInfo]bool // Stores all goroutines that still hold reference to this channel
	strDebug        string
	intFlagFoundBug int32 // Use atomic int32 operations to mark if a bug is reported
}

var MapChan2ChanInfo map[interface{}]*ChanInfo // Help us retrieve ChanInfo with a given channel
...
func FindChanInfo(ch interface{}) *ChanInfo {
	...
	if chInfo, ok := MapChan2ChanInfo[ch]; ok {
		return chInfo
	}

	print("Warning in FindChanInfo: no ChanInfo found for ch")
	return nil
}
...
func (chInfo *ChanInfo) AddGoroutine(goInfo *GoInfo) {
	chInfo.mapRefGoroutine[goInfo] = true
}
...
func ReportBug(chInfo *ChanInfo) {
	...
	if PrintBug {
		print("A blocking bug is found for channel: ", chInfo.strDebug)
		print("\tThere are %d goroutines blocked at this channel\n", countRefGo)
		panic(1)
	}
}
...
type GoInfo struct {
	...
	mapChanInfo map[*ChanInfo]bool // Stores all channels that this goroutine still hold reference to
...
var MapID2GoInfo map[int64]*GoInfo
...
func NewGoroutine() *GoInfo {
	...
	//_, strFileName, intLine, ok := runtime.Caller(1)
	//if !ok {
	//	print("Fatal error in NewGoroutine: can't find the caller")
	//	return newGoInfo
	//}
	//strPosition := strFileName + ":" + strconv.Itoa(intLine)
...
func FindGoInfo(id int64) *GoInfo {
	goInfo, ok := MapID2GoInfo[id]
	if ok {
		return goInfo
	} else {
		print("Warning in FindGoInfo: no GoInfo found for the given id:", id)
	}
...
func (goInfo *GoInfo) AddChan(chInfo *ChanInfo) {
	goInfo.mapChanInfo[chInfo] = true
}
...
func (goInfo *GoInfo) IsBlock() (boolIsBlock bool, strOp string) {
	...
	if len(mapBlock) > 1 {
		...
		print("Fatal in GoInfo.IsBlock(): mapBlock is not nil but lenth is 0")
	}
...
//	if err != nil {
//		print("Fatal in PrintLogger: ", err)
//		return
//	}
//	print(jsonLogger)
//}
...
//	_, strFileName, intLine, ok := Caller(1)
//	if !ok {
//		print("Fatal error in RecordSleep: can't find the caller")
//		return
//	}
//	strPosition := strFileName + ":" + string(intLine)
//	newOpLog := LogSleep{
